...
| 项目 | 原稿 | 建议 |
| --- | --- | --- |
| 时间筛选 | This Month / This Month / 24 Hours | 本月 / 本周 / 近24小时（按应用语言翻译） |
...
- 明确时间窗口：本月 = 本地时区月初至当前；本周 = 本地时区周一 00:00 至当前；24 Hours = 当前时间回溯真实 24 小时。每次请求固定一个 now，不用“30 天”代替自然月。API 24/25 使用 core library desugaring 或兼容时间 API。

...
class StartupNavigationTest {
    ...
    fun systemSplashUsesApplicationIconAndHomeUsesScopedFade() {
        ...
        val enter =
            android.view.animation.AnimationUtils.loadAnimation(context, R.anim.startup_home_enter)
        val exit =
            android.view.animation.AnimationUtils.loadAnimation(context, R.anim.startup_home_exit)
    ...
    @Test
    fun launcherAndModuleNotificationsResolveToStartupActivity() {
        assertEquals(
        ...
        assertEquals(
            StartupActivity::class.java.name,
            entryPointIntent(context).component?.className,
        )
    }

    @Test
    fun allNotificationTargetsSurviveStartupAndReachHomeOnce() {
        for (target in NotificationDestination.entries) {
            val fromModule =
                entryPointIntent(context)
                    .putExtra(LANDING_NOTIFICATION_ACTION, target.contentType)
            ...
            assertTrue(home.getBooleanExtra(StartupNavigation.PERMISSION_COMPLETED, false))
    ...
    @Test
    fun legacyMainNotificationsRedirectAndFirebaseConsolePayloadUsesWhitelist() {
        val legacy =
            Intent(context, MainActivity::class.java)
                .putExtra(NotificationNavigation.EXTRA_DESTINATION, "network")
        assertTrue(StartupNavigation.needsStartup(legacy))
        val console =
            Intent().putExtra("google.message_id", "test").putExtra("destination", "photos")
    ...
    @Test
    fun permissionReturnTargetsTheProvidedActivityInsteadOfAlwaysHome() {
        for (host in
            listOf(
                StartupActivity::class.java,
                MainActivity::class.java,
                com.example.aicleanphonestorage.feature.settings.AboutActivity::class.java,
            )) {
            val component = android.content.ComponentName(context, host)
            val intent =
                com.example.aicleanphonestorage.core.permissions.PermissionSettingsNavigator
                    .returnIntent(component, "push-request")
            ...
            assertFalse(StartupNavigation.needsStartup(intent))
        }
    }
}

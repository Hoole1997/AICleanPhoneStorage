...
internal class NotificationCleanerRenderer(
    ...
    init {
        ...
        binding.notificationList.itemAnimator = DefaultItemAnimator().apply { supportsChangeAnimations = false }
    ...
    fun render(state: NotificationUiState) {
        ...
        if (state.saveError > lastSaveError) {
            ...
            Snackbar.make(binding.root, R.string.notification_save_failed, Snackbar.LENGTH_LONG).show()
        }
    }
    fun setActive(value: Boolean) = adapter.setActive(value)
    fun dispose() { adapter.setActive(false); binding.notificationList.adapter = null }
}

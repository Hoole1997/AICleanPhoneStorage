 ...
 */
internal class EntryLoadingProgress(
    private val startedAt: Long,
    private val durationMillis: Long,
    private val monotonicMillis: () -> Long,
) {
    data class Frame(val detail: TrafficProgress, val percent: Int?)
    private data class Actual(val detail: TrafficProgress, val finished: Boolean = false)
    private val actual = AtomicReference(Actual(TrafficProgress(TrafficStage.MOBILE)))
    ...
    fun report(progress: TrafficProgress) {
        actual.updateAndGet { if (it.finished) it else Actual(progress) }
    }
    fun complete(appCount: Int) {
        actual.set(Actual(TrafficProgress(TrafficStage.APPLICATIONS, appCount, appCount), finished = true))
    }
